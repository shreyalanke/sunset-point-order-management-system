...
function OrdersPage() {
  ...
  const filteredOrders = orders.filter(order => {
    const matchesSearch = order.tag?.toLowerCase().includes(searchQuery.toLowerCase()) || 
  ...
  const handleUpdateQuantity = (itemId, newQuantity) => {
    ...
    setOrders(prev => prev.map(order => ({
      ...order,
      items: order.items.map(item => item.id === itemId ? { ...item, quantity: newQuantity } : item)
    })));
  };
    ...
    setOrders(prev => prev.map(order => {
      ...
      return {
        ...
        items: order.items.map(item => 
          item.id === itemId ? { ...item, status: item.status === 'served' ? 'pending' : 'served' } : item
        )
      };
    }));
  };
  ...
  const handleTogglePayment = (orderId) => {
    setOrders(prev => prev.map(order => 
      order.id === orderId ? { ...order, paymentDone: !order.paymentDone } : order
    ));
  };

  const openConfirmDialog = (type, orderId) => setConfirmDialog({ show: true, type, orderId });
              ...
              visibleOrders.map(order => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && (
                       <ChevronRight className="absolute right-2 top-1/2 -translate-y-1/2 text-white/20" size={40} />
                    )}
                  </div>
                );
              })
          ...
          {selectedOrderId ? (
            <div className="h-full overflow-y-auto pb-20 custom-scrollbar pr-1">
              <OrderCard
                ...
                onCancelOrder={() => openConfirmDialog('cancel', selectedOrderId)}
            ...
            <div className="h-full flex flex-col items-center justify-center bg-white/50 rounded-2xl border-2 border-dashed border-gray-200 text-center p-8">
              ...
              <p className="text-gray-500 max-w-sm">
                Select an order from the list on the left to view details, manage items, or process payments.
              </p>
            </div>
          )}
        </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-[2px]">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden transform transition-all scale-100">
            <div className="p-6 text-center">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4 ${confirmDialog.type === 'close' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                {confirmDialog.type === 'close' ? <CheckCircle2 size={24} /> : <AlertCircle size={24} />}
              </div>
              <h3 className="text-lg font-bold text-gray-900 mb-2">
                {confirmDialog.type === 'close' ? 'Complete Order?' : 'Cancel Order?'}
              </h3>
  ...
  return (
    <div className="bg-white rounded-xl p-3 sm:p-4 border border-gray-100 shadow-sm flex items-center gap-3 sm:gap-4 transition-transform hover:scale-[1.02]">
      ...
      <div className="min-w-0">
        ...
        <p className="text-lg sm:text-xl font-black text-gray-900 truncate">{value}</p>
      </div>
    </div>
  );
}
